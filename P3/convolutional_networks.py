...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        x_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad), mode='constant', value=0)
        dx_padded = torch.zeros_like(x_padded, dtype=x.dtype, device=x.device)
            ...
            for c in range(C):
                for h_out in range(H_prime):
                    for w_out in range(W_prime):
                        ...
                        out[n, c, h_out, w_out] = torch.max(x_slice)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_out, hidden_cache = Linear_ReLU.forward(conv_out.reshape(X.shape[0], -1), W2, b2)
        scores, score_cache = Linear.forward(hidden_out, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dhidden, grads['W3'], grads['b3'] = Linear.backward(dscores, score_cache)
        dconv_reshaped, grads['W2'], grads['b2'] = Linear_ReLU.backward(dhidden, hidden_cache)
        dconv = dconv_reshaped.reshape(conv_out.shape)
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dconv, conv_cache)
...
class DeepConvNet(object):
    def __init__(self, input_dims=(3, 32, 32), num_filters=[8, 8, 8, 8, 8], max_pools=[0, 1, 2, 3, 4],
                 batchnorm=False, num_classes=10, weight_scale=1e-3, reg=0.0, weight_initializer="kaiming",
        ...
        for i in range(L):
            filter_num = num_filters[i]
            if weight_scale == "kaiming":
                self.params[f'W{i+1}'] = kaiming_initializer(C, filter_num, K=3, relu=True, device=device, dtype=dtype)
            else:
                self.params[f'W{i+1}'] = weight_scale * torch.randn(filter_num, C, 3, 3, dtype=dtype, device=device)

            self.params[f'b{i+1}'] = torch.zeros(filter_num, dtype=dtype, device=device)
            
            if batchnorm:
                self.params[f'gamma{i+1}'] = torch.ones(filter_num, dtype=dtype, device=device)
                self.params[f'beta{i+1}'] = torch.zeros(filter_num, dtype=dtype, device=device)
        ...
        self.params[f'W{L+1}'] = kaiming_initializer(C * H * W, num_classes, device=device, dtype=dtype)
        self.params[f'b{L+1}'] = torch.zeros(num_classes, dtype=dtype, device=device)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(
        ...
        update_rule=adam,
        optim_config={'learning_rate': 5e-3, 'momentum': 0.5},
        lr_decay=0.95,
