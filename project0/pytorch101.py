...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    """
    N = len(x)
    C = max(x) + 1 if x else 0
    indices = torch.tensor(x, dtype=torch.int64).unsqueeze(1)
    y = torch.zeros((N, C), dtype=torch.float32).scatter_(1, indices, 1.0)
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    """
    y = torch.cat(
        [x[:12].view(3, 4), x[12:].view(3, 4)], dim=1
    )
    return y
def zero_row_min(x: Tensor) -> Tensor:
    # Find the minimum value indices per row and clone x for output
    row_min_indices = torch.argmin(x, dim=1)
    y = x.clone()
    y[torch.arange(x.size(0)), row_min_indices] = 0
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    # Explicit loop over batch dimension B
    B, N, M = x.shape
    _, _, P = y.shape
...
def batched_matrix_multiply_noloop(x: Tensor, y: Tensor) -> Tensor:
    # Efficient batch matrix multiplication using torch.bmm
    return torch.bmm(x, y)

def normalize_columns(x: Tensor) -> Tensor:
    column_means = x.mean(dim=0)
    column_stds = x.std(dim=0, unbiased=True)

    normalized_x = (x - column_means) / column_stds
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    # Move tensors to GPU, perform multiplication, and return to CPU
    x_gpu = x.cuda()
    w_gpu = w.cuda()
