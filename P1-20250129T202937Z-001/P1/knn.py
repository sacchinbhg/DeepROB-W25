...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            ...
            squared_dist = torch.sum(diff**2)
            dists[i, j] = torch.sqrt(torch.clamp(squared_dist, min=0.0))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        squared_dists = torch.sum(diff**2, dim=list(range(1, x_test.ndim)))
        dists[i, :] = torch.sqrt(squared_dists)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat = x_test.reshape(x_test.size(0), -1)
    train_square = x_train_flat.pow(2).sum(dim=1, keepdim=True)
    test_square = x_test_flat.pow(2).sum(dim=1, keepdim=True)
    cross_term = -2 * x_train_flat @ x_test_flat.T
    dists = train_square + test_square.T + cross_term
    dists = dists.sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, closest_y = torch.topk(dists[:, i], k=k, largest=False)
        k_nearest_neighbors = y_train[closest_y]
        label_counts = torch.bincount(k_nearest_neighbors)
        max_count = torch.max(label_counts)
        max_count_labels = torch.where(label_counts == max_count)[0]
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            ...
            # Train the classifier and compute accuracy
            classifier = KnnClassifier(train_x, train_y)
            accuracy = classifier.check_accuracy(val_x, val_y, k)
