...
def svm_loss(x, y):
    ...
    # Replace "pass" statement with your code
    N = x.shape[0]
    correct_class_scores = x[torch.arange(N), y].unsqueeze(1)
    margins = torch.clamp(x - correct_class_scores + 1, min=0)
...
def softmax_loss(x, y):
    ...
    Z = torch.sum(torch.exp(shifted_logits), dim=1, keepdim=True)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        h1 = torch.nn.functional.relu(X.view(X.shape[0], -1).mm(self.params['W1']) + self.params['b1'])

        scores = h1.mm(self.params['W2']) + self.params['b2']
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dh1 = dscores.mm(self.params['W2'].t())
        dh1[h1 <= 0] = 0  # ReLU gradient
        grads['W1'] = X.view(X.shape[0], -1).t().mm(dh1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params[f'W{self.num_layers}'] = torch.randn(layer_dim, num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # Hidden Layers (Linear - ReLU - Dropout (if True))
        for i in range(1, self.num_layers):
          W, b = self.params[f'W{i}'], self.params[f'b{i}']
          h, cache[f'layer{i}'] = Linear_ReLU.forward(h, W, b)
          if self.use_dropout: 
              h, dropout_cache[f'dropout{i}'] = Dropout.forward(h, self.dropout_param)
              
        W_final, b_final = self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}']
        scores, cache[f'layer{self.num_layers}'] = Linear.forward(h, W_final, b_final)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, cache[f'layer{self.num_layers}'])
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']
        
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_cache[f'dropout{i}'])
            dout, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dout, cache[f'layer{i}'])
